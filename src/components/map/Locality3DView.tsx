      ...
      try {
        ...
        liveBuildings.forEach((b) => {
          for (let i = 0; i < 14; i++) {
            ...
            m.position.set(
              b.x + Math.cos(phase) * r,
              b.h + 20 + Math.random() * 40,
              b.y + Math.sin(phase) * r,
            );
            viewerGroup.add(m);
        ...
        <div className="relative flex-1">
          ...
          <div className="pointer-events-none absolute left-3 top-3 grid grid-cols-2 gap-2">
            <Stat
              ...
              label="Viewers / 7d"
              value={locality.totalViewersLast7d.toLocaleString()}
            />
            <Stat
              icon={<Users className="h-3 w-3" />}
              label="Live now"
              value={String(locality.liveSessions)}
              accent="live"
            />
            <Stat
              icon={<Flame className="h-3 w-3" />}
              label="Hotness"
              value={`${locality.hotness}/100`}
              accent="gold"
            />
            <Stat
              icon={<Building2 className="h-3 w-3" />}
              label="Inventory"
              value={`${locality.inventoryUnits} units`}
            />
          </div>
          ...
          <div className="pointer-events-none absolute bottom-3 left-3 right-3 flex items-end justify-between gap-2 text-[10px] text-white/60">
            ...
            <div className="rounded-lg border border-white/10 bg-white/5 px-2 py-1 backdrop-blur">
              <Maximize2 className="mr-1 inline h-3 w-3" />₹{locality.avgPriceCr} Cr avg ·{" "}
              {locality.pricePerSqftK}k psf · +{locality.yoyAppreciation}% YoY
            </div>
          </div>
        </div>
      </div>
