      ...
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-6 px-4 text-center">
        <div className="rounded-2xl border border-border bg-card p-8 shadow-[var(--shadow-soft)] max-w-sm w-full">
          ...
          {!leftDueToEnded ? (
            <>
              <Button
                onClick={() => { setHasLeft(false); setLeftDueToEnded(false); }}
  ...
  return (
    <LiveRoom room={ROOM_NAME} identity={identity} role="viewer">
      ...
      <LiveLayout
        ...
        onStreamEnded={() => { setLeftDueToEnded(true); setHasLeft(true); }}
      />
    </LiveRoom>
  );
}

function LiveLayout({ onLeave, onStreamEnded }: { onLeave: () => void; onStreamEnded: () => void }) {
    ...
    <div className="grid gap-4 px-4 py-6 md:px-8 lg:grid-cols-[1fr_380px]">
      <div className="space-y-4">
        <div
          ...
          style={{ background: "linear-gradient(135deg, oklch(0.25 0.06 250), oklch(0.42 0.15 245))" }}
        >
          {mainTrack && !streamEnded ? (
            <ParticipantTile trackRef={mainTrack} disableSpeakingIndicator className="absolute inset-0 h-full w-full" />
          ) : (
            <>
                ...
                {streamEnded ? (
                  <div className="flex flex-col items-center gap-3 rounded-2xl bg-black/60 px-6 py-5 backdrop-blur text-center">
                    ...
                    <div>
                      ...
                      <div className="mt-0.5 text-xs text-white/60">The host is no longer broadcasting.</div>
                    </div>
                    <button
          ...
          {screenTrack && cameraTrack && !streamEnded && (
            <div className="absolute bottom-14 right-3 h-24 w-40 overflow-hidden rounded-xl border-2 border-white/20 shadow-lg">
              <ParticipantTile trackRef={cameraTrack} disableSpeakingIndicator className="h-full w-full" />
            </div>
          )}
          ...
          <div className="absolute left-4 top-4 flex items-center gap-2">
            ...
            <Badge variant="secondary" className="bg-black/40 px-2 py-1 text-[11px] text-white backdrop-blur">
            ...
            <div className="glass flex items-center gap-3 rounded-xl px-3 py-2">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 text-sm font-bold">
                  ...
                  <span className="text-[11px] font-medium text-muted-foreground">· Locality Legend · Sector 65</span>
                </div>
              </div>
            </div>
            <div className="glass rounded-xl px-3 py-2 text-xs">
              <span className="font-semibold">{screenTrack ? "Presentation" : "Drone Tour"}</span> · Live
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-border bg-card p-3 shadow-[var(--shadow-soft)]">
          ...
          <Button variant="outline" className="gap-2 border-border bg-card text-foreground hover:bg-accent"><FileText className="h-4 w-4" />Download RERA Docs</Button>
        ...
        <div className="rounded-2xl border border-border bg-card p-4 shadow-[var(--shadow-soft)]">
          ...
          <div className="space-y-2">
            {[{ k: "Yes, much better", v: 62 }, { k: "About the same", v: 24 }, { k: "No, Privana wins", v: 14 }].map((o) => {
              const sel = poll === o.k;
              return (
                <button key={o.k} onClick={() => setPoll(o.k)} className="relative w-full overflow-hidden rounded-lg border border-border bg-card px-3 py-2 text-left text-sm">
                  <div className={`absolute inset-y-0 left-0 rounded-lg ${sel ? "bg-primary/15" : "bg-secondary/60"}`} style={{ width: `${o.v}%` }} />
          ...
          <div className="mx-4 w-full max-w-sm rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-elegant)]">
            ...
            <div className="mt-5 flex gap-2">
              ...
              <Button
                ...
                onClick={() => { setShowLeaveConfirm(false); onLeave(); navigate({ to: "/demand" }); }}
              >
                Leave
              </Button>
            </div>
    ...
    <aside className="flex h-[calc(100vh-180px)] min-h-[560px] flex-col rounded-2xl border border-border bg-card shadow-[var(--shadow-soft)]">
      ...
      <div className="flex-1 space-y-2 overflow-y-auto p-3">
        <div className="rounded-lg border border-primary/20 bg-primary/5 p-2.5">
          <div className="mb-1 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-primary">
            <Pin className="h-3 w-3" />Pinned by Host
          </div>
          <div className="text-xs text-foreground">Floor plans + RERA docs are linked below. Ask any question — Anika verifying answers live.</div>
        </div>
        ...
        {questions.length > 0 && (
          <div className="rounded-lg border border-amber-200/40 bg-amber-50/10 p-2.5">
            <div className="mb-1.5 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-amber-600">
              <HelpCircle className="h-3 w-3" />{questions.length} question{questions.length > 1 ? "s" : ""} from viewers
            </div>
        ...
        {regularChat.length === 0 && questions.length === 0 && (
          <div className="rounded-lg bg-secondary p-2.5 text-xs text-muted-foreground">No messages yet. Be the first to ask.</div>
        )}
        ...
        {regularChat.map((m) => {
          ...
          return (
            <div key={m.id ?? name} className={`rounded-lg p-2.5 ${isHost ? "bg-[var(--gold)]/10 border-l-2 border-[var(--gold)]" : ""}`}>
              <div className="mb-1 flex items-center gap-1.5">
                <div className={`grid h-6 w-6 place-items-center rounded-full text-[10px] font-bold ${isHost ? "bg-[var(--gold)] text-[var(--gold-foreground)]" : "bg-secondary"}`}>{initial}</div>
                <span className="text-xs font-semibold text-foreground">{name}</span>
                {isHost && <Badge className="gap-1 bg-[var(--gold)] px-1.5 py-0 text-[9px] text-[var(--gold-foreground)]"><Sparkles className="h-2.5 w-2.5" />Host</Badge>}
                {m.from?.isLocal && <Badge className="bg-primary/10 px-1.5 py-0 text-[9px] text-primary">You</Badge>}
              </div>
      ...
      <div className="border-t border-border p-3">
        <div className="flex items-center gap-2 rounded-xl border border-border bg-card px-3 py-2">
          ...
          />
          <button onClick={handleSend} disabled={isSending} className="grid h-7 w-7 place-items-center rounded-md bg-primary text-primary-foreground disabled:opacity-50">
